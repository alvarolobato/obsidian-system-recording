...
- When a calendar meeting starts (or ends while recording), you get a meeting prompt on **one** channel: an **in-app notice** when Obsidian is frontmost, or a **native macOS notification** (with a primary action — *Join & record* / *Record* / *Stop recording*) when it isn't. Bringing Obsidian to the front closes the OS banner and shows the in-app notice with the full set of choices (*Join*, *Open note*, …). A new prompt replaces any previous one so they don't stack. If you're already recording, Join & record / Record labels note that they stop the current take.
